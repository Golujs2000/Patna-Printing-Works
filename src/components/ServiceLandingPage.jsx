...
export default function ServiceLandingPage({ onSubmitEstimate }) {
  const { serviceId: urlServiceId } = useParams();
  const navigate = useNavigate();
  
  const activeServiceId = urlServiceId || 'doctor-file';
  const service = serviceDetailsMap[activeServiceId] || serviceDetailsMap['doctor-file'];

  const [activeMockupIndex, setActiveMockupIndex] = useState(0);
  const [activeTab, setActiveTab] = useState('specs');
  const [expandedFaq, setExpandedFaq] = useState(0);
  const [isLightboxOpen, setIsLightboxOpen] = useState(false);
  const [prefilledService, setPrefilledService] = useState(service.title);
  const [prefilledDetails, setPrefilledDetails] = useState(
    `Hi! I would like to get a quote and proof sample for ${service.title} (${service.specs.minQuantity} min order).`
  );

  // Sync title and reset state when route changes
  useEffect(() => {
    setActiveMockupIndex(0);
    setPrefilledService(service.title);
    setPrefilledDetails(`Hi! I would like to get a quote and proof sample for ${service.title} (${service.specs.minQuantity} min order).`);
    document.title = `${service.title} | Patna Printing Works`;
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [activeServiceId, service.title]);
...
Please provide a quote and sample proofs.`;
  ...
  const handleScrollToCalculator = () => {
    document.getElementById('service-calc-section')?.scrollIntoView({ behavior: 'smooth' });
  };

  const handleScrollToForm = () => {
    document.getElementById('service-inquiry-section')?.scrollIntoView({ behavior: 'smooth' });
  };

  const handleEstimatePassToForm = (estimateData) => {
    setPrefilledService(estimateData.service);
    setPrefilledDetails(estimateData.details);
    if (onSubmitEstimate) {
      onSubmitEstimate(estimateData);
    }
    setTimeout(() => {
      document.getElementById('service-inquiry-section')?.scrollIntoView({ behavior: 'smooth' });
    }, 100);
  ...
  return (
    <div className="w-full bg-slate-50 min-h-screen font-sans">
      ...
      <div className="bg-white border-b border-slate-200 py-4 shadow-sm sticky top-[65px] z-40">
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-wrap justify-between items-center gap-3">
          
          {/* Back to Home Button */}
          <Link
            to="/"
            className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-800 px-3.5 py-1.5 rounded-xl border border-slate-200 text-xs font-bold transition-all shadow-xs"
          >
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
            ...
            <span>/</span>
            <Link to="/" className="cursor-pointer hover:text-blue-600 transition-colors">Services</Link>
            <span>/</span>
            <span className="font-bold text-blue-600 truncate max-w-[200px] sm:max-w-none">{service.title}</span>
          </div>

          {/* Quick Call Header CTA */}
          <a
            href={`tel:${businessDetails.phoneDial}`}
            className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-slate-700 hover:text-blue-600"
          >
            <PhoneCall className="w-3.5 h-3.5 text-emerald-600" />
            <span>Instant Quote: {businessDetails.phone}</span>
          </a>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          {/* Left Column: Interactive HD Gallery (7 cols) */}
          <div className="lg:col-span-7 flex flex-col gap-4">
            ...
            <div className="relative bg-white rounded-2xl border border-slate-200 p-4 shadow-xl group overflow-hidden">
              <div 
                ...
                className="relative w-full h-[340px] sm:h-[450px] bg-slate-100 rounded-xl overflow-hidden border border-slate-200 flex items-center justify-center cursor-pointer"
              >
                ...
                <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
                  <span className="font-sans text-[10px] bg-blue-600 text-white px-3 py-1 rounded-md font-bold uppercase shadow-md">
                    {service.category}
                  </span>
                  <span className="font-sans text-[10px] bg-emerald-600 text-white px-3 py-1 rounded-md font-bold uppercase shadow-md">
                ...
                <button
                  onClick={() => setIsLightboxOpen(true)}
                  className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-slate-800 p-2.5 rounded-xl border border-slate-200 shadow-md hover:bg-white transition-all"
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-3 text-left">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="font-sans text-[10px] text-blue-600 font-bold uppercase bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
                    HD Proof
                  </span>
                </div>
                ...
                <button
                  ...
                  className={`text-left p-2 rounded-xl border transition-all relative overflow-hidden ${
                    activeMockupIndex === idx
                      ...
                      : 'bg-white border-slate-200 opacity-75 hover:opacity-100 hover:bg-slate-50'
                  }`}
                >
          ...
          <div className="lg:col-span-5 flex flex-col justify-between text-left bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-xl">
            ...
            <div>
              ...
              {/* Service Headline */}
              <h1 className="font-sans font-black text-2xl sm:text-3xl lg:text-4xl text-slate-900 leading-tight mb-2">
                {service.title}
              </h1>
              <p className="font-sans font-bold text-xs sm:text-sm text-blue-600 mb-4">
                {service.subtitle}
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                  <span className="font-sans text-[10px] uppercase font-bold text-slate-400 block">PRINT PRESS</span>
              ...
              <div className="space-y-2 mb-6">
                <span className="font-sans font-bold text-xs text-slate-700 uppercase tracking-wider block mb-2">
                  KEY FEATURES & SERVICE GUARANTEES
                </span>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100 mt-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="bg-blue-50 hover:bg-blue-100 text-blue-700 py-3 px-3 rounded-xl font-sans font-bold text-xs border border-blue-200 flex items-center justify-center gap-2 transition-all"
                >
                ...
                <button
                  ...
                  className="bg-slate-800 hover:bg-slate-900 text-white py-3 px-3 rounded-xl font-sans font-bold text-xs border border-slate-800 flex items-center justify-center gap-2 transition-all"
                >
        ...
        {/* Technical Specs & Rate Matrix Tabs Section */}
        ...
        {/* Embedded Live Price Calculator prefilled for this service */}
        <div id="service-calc-section" className="mb-12">
          <Calculator 
            initialServiceId={service.id} 
            onSubmitEstimate={handleEstimatePassToForm} 
          />
        </div>

        {/* Embedded Lead Inquiry Form prefilled for this service */}
        <div id="service-inquiry-section" className="mb-12">
          <InquiryForm 
        ...
        {/* Explore Other Service Landing Pages */}
        <div className="border-t border-slate-200 pt-10 text-left">
          <h4 className="font-sans font-black text-xl text-slate-900 uppercase mb-6 flex items-center gap-2">
            <PackageCheck className="w-6 h-6 text-blue-600" />
            <span>Explore Other Service Landing Pages</span>
          </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {services.filter(s => s.id !== service.id).map((other) => (
              <Link
                ...
                className="bg-white p-4 rounded-xl border border-slate-200 hover:border-blue-500 text-left transition-all shadow-sm hover:shadow-md group flex flex-col justify-between"
              >
                <div>
                  <span className="font-sans text-[10px] font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded inline-block mb-2">
                    {other.category}
                  </span>
                  <h5 className="font-sans font-bold text-xs text-slate-900 group-hover:text-blue-600 transition-colors">
                    {other.title}
                  </h5>
                </div>
                <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-100 font-sans text-[11px] font-bold text-slate-500 group-hover:text-blue-600">
